...
contract GoldVeinPair is ERC20, BoringOwnable, IMasterContract {
    ...
    GoldVeinPair public immutable masterContract;
    ...
    function name() external view returns (string memory) {
        return string(abi.encodePacked("GoldVein Medium Risk ", collateral.safeName(), "/", asset.safeName(), "-", oracle.name(oracleData)));
    }
    ...
    /// @dev Helper function for depositing into `alPine`.
    function _alpDeposit(
        bytes memory data,
        uint256 value,
        uint256 value1,
        uint256 value2
        ...
        return alPine.deposit{value: value}(token, msg.sender, to, uint256(amount), uint256(share));
    }

    /// @dev Helper function to withdraw from the `alPine`.
    function _alpWithdraw(
        bytes memory data,
        uint256 value1,
        uint256 value2
    ) internal returns (uint256, uint256) {
        (IERC20 token, address to, int256 amount, int256 share) = abi.decode(data, (IERC20, address, int256, int256));
        return alPine.withdraw(token, msg.sender, to, _num(amount, value1, value2), _num(share, value1, value2));
    }

    /// @dev Helper function to perform a contract call and eventually extracting revert messages on failure.
    /// Calls to `alPine` are not allowed for obvious security reasons.
    ...
    function _call(
        ...
        require(callee != address(alPine) && callee != address(this), "GoldVeinPair: can't call");

        (bool success, bytes memory returnData) = callee.call{value: value}(callData);
        require(success, "GoldVeinPair: call failed");
    ...
    function cook(
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action == ACTION_ADD_COLLATERAL) {
                ...
                value1 = alPine.toShare(asset, totalBorrow.toElastic(_num(part, value1, value2), true), true);
    ...
    function liquidate(
        ...
        Rebase memory _totalBorrow = totalBorrow;
        Rebase memory alPineTotals = alPine.totals(collateral);
        ...
        require(allBorrowAmount != 0, "GoldVeinPair: all are solvent");
        ...
        uint256 allBorrowShare = alPine.toShare(asset, allBorrowAmount, true);

        if (!open) {
            // Closed liquidation using a pre-approved swapper for the benefit of the LPs
            require(masterContract.swappers(swapper), "GoldVeinPair: Invalid swapper");

            // Swaps the users' collateral for the borrowed asset
            alPine.transfer(collateral, address(this), address(swapper), allCollateralShare);
            swapper.swap(collateral, asset, address(this), allBorrowShare, allCollateralShare);

            uint256 returnedShare = alPine.balanceOf(asset, address(this)).sub(uint256(totalAsset.elastic));
            ...
            alPine.transfer(asset, address(this), masterContract.feeTo(), feeShare);
            ...
            alPine.transfer(collateral, address(this), swapper == ISwapper(0) ? to : address(swapper), allCollateralShare);
            if (swapper != ISwapper(0)) {
                swapper.swap(collateral, asset, msg.sender, allBorrowShare, allCollateralShare);
            }

            alPine.transfer(asset, msg.sender, address(this), allBorrowShare);
