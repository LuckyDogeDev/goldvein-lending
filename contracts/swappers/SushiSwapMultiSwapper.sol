...
contract LuckySwapMultiSwapper {
    ...
    constructor (address _factory, IAlpineV1 _alPine, bytes32 _pairCodeHash) public {
        factory = _factory;
        alPine = _alPine;
    ...
    function swap (IERC20 tokenIn, IERC20 tokenOut, uint256 amountMinOut, address[] calldata path, uint256 shareIn) external returns (uint256 amountOut, uint256 shareOut) {
        (uint256 amountIn, ) = alPine.withdraw(tokenIn, address(this), address(this), 0, shareIn);
        amountOut = _swapExactTokensForTokens(amountIn, amountMinOut, path, address(alPine));
        (, shareOut) = alPine.deposit(tokenOut, address(alPine), msg.sender, amountOut, 0);
    }
