...
contract LuckySwapMultiSwapper {
    ...
    function getOutputAmount(
        ...
        uint256 amountIn = alPine.toAmount(tokenIn, shareIn, false);
    ...
    function swap(
        ...
        (uint256 amountIn, ) = alPine.withdraw(tokenIn, address(this), UniswapV2Library.pairFor(factory, path[0], path[1], pairCodeHash), 0, shareIn);
        uint256 amount = _swapExactTokensForTokens(amountIn, amountMinOut, path, address(alPine));
        (, uint256 share) = alPine.deposit(tokenOut, address(alPine), to, amount, 0);
