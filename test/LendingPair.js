...
async function debugInfo(thisObject) {
    console.log("Alice Collateral in Alp", (await thisObject.alPine.balanceOf(thisObject.a.address, thisObject.alice.address)).toString())
    console.log("Bob Collateral in Alp", (await thisObject.alPine.balanceOf(thisObject.a.address, thisObject.bob.address)).toString())
    console.log(
    ...
    console.log(
        ...
        (await thisObject.alPine.balanceOf(thisObject.a.address, thisObject.pairHelper.contract.address)).toString()
    )
    console.log()
    ...
    console.log(
        "Pair Asset in Alp",
        (await thisObject.alPine.balanceOf(thisObject.b.address, thisObject.pairHelper.contract.address)).toString()
    )
    console.log()
...
describe("GoldVeinPair Basic", function () {
    before(async function () {
        fixture = await createFixture(deployments, this, async (cmd) => {
            ...
            await cmd.addGoldVeinPair("pairHelper", this.alPine, this.goldveinPair, this.a, this.b, this.oracle, oracleData)
            ...
            await this.alPine.connect(this.fred).addProfit(this.a.address, getBigNumber(30))

            await this.b.connect(this.fred).approve(this.alPine.address, getBigNumber(400, 8))
            await expect(this.alPine.connect(this.fred).deposit(this.b.address, this.fred.address, this.fred.address, getBigNumber(200, 8), 0))
                .to.emit(this.b, "Transfer")
                .withArgs(this.fred.address, this.alPine.address, getBigNumber(200, 8))
                .to.emit(this.alPine, "LogDeposit")
                .withArgs(this.b.address, this.fred.address, this.fred.address, getBigNumber(200, 8), getBigNumber(200, 8))

            await this.alPine.connect(this.fred).addProfit(this.b.address, getBigNumber(200, 8))
        })
    })
    ...
    describe("Deployment", function () {
        it("Assigns a name", async function () {
            expect(await this.pairHelper.contract.name()).to.be.equal("GoldVein Medium Risk Token A/Token B-Test")
        })
    ...
    describe("Init", function () {
        it("Reverts init for collateral address 0", async function () {
            const oracleData = await this.oracle.getDataParameter()
            await expect(
                cmd.addGoldVeinPair("pairHelper", this.alPine, this.goldveinPair, ADDRESS_ZERO, this.b, this.oracle, oracleData)
            ).to.be.revertedWith("GoldVeinPair: bad pair")
        })

        it("Reverts init for initilised pair", async function () {
            await expect(this.pairHelper.contract.init(this.pairHelper.initData)).to.be.revertedWith("GoldVeinPair: already initialized")
        })
    })
    ...
    describe("Add Asset", function () {
        it("should add asset with skim", async function () {
            await this.b.approve(this.alPine.address, getBigNumber(2, 8))
            await this.alPine.deposit(this.b.address, this.alice.address, this.alice.address, 0, getBigNumber(1, 8))
            await this.alPine.transfer(this.b.address, this.alice.address, this.pairHelper.contract.address, getBigNumber(1, 8))
        ...
        it("should revert when trying to skim too much", async function () {
            await this.b.approve(this.alPine.address, getBigNumber(2))
            await this.alPine.deposit(this.b.address, this.alice.address, this.alice.address, 0, getBigNumber(1, 8))
            await this.alPine.transfer(this.b.address, this.alice.address, this.pairHelper.contract.address, getBigNumber(1, 8))
            await expect(
                this.pairHelper.run((cmd) => [cmd.do(this.pairHelper.contract.addAsset, this.alice.address, true, getBigNumber(2, 8))])
            ).to.be.revertedWith("GoldVeinPair: Skim too much")
        })

        it("should revert if MasterContract is not approved", async function () {
            await this.b.connect(this.carol).approve(this.alPine.address, 300)
            await expect((await this.pairHelper.as(this.carol)).depositAsset(290)).to.be.revertedWith("Alpine: Transfer not approved")
        })

        it("should take a deposit of assets from Alpine", async function () {
        ...
        it("should emit correct event on adding asset", async function () {
            await this.b.approve(this.alPine.address, 3000)
    ...
    describe("Add Collateral", function () {
        it("should take a deposit of collateral", async function () {
            await this.a.approve(this.alPine.address, 300)
    ...
    describe("Remove Collateral", function () {
        ...
        it("should allow a direct removal of collateral", async function () {
            ...
            expect(await this.alPine.balanceOf(this.a.address, this.alice.address)).to.be.equal(getBigNumber(50))
        })
        ...
        it("should not allow a remove of collateral if user is insolvent", async function () {
            ...
            await expect(this.pairHelper.withdrawCollateral(getBigNumber(1, 0))).to.be.revertedWith("GoldVeinPair: user insolvent")
        })
    ...
    describe("Borrow", function () {
        it("should not allow borrowing without any assets", async function () {
            await expect(this.pairHelper.contract.borrow(this.alice.address, 10000)).to.be.revertedWith("GoldVein: below minimum")
            await expect(this.pairHelper.contract.borrow(this.alice.address, 1)).to.be.revertedWith("GoldVein: below minimum")
        })

        it("should not allow borrowing without any collateral", async function () {
            await this.b.approve(this.alPine.address, 300)
    ...
    describe("Repay", function () {
        ...
        it("should allow to repay from Alpine", async function () {
            await this.pairHelper.run((cmd) => [
                cmd.approveAsset(getBigNumber(700, 8)),
                cmd.depositAsset(getBigNumber(290, 8)),
                cmd.approveCollateral(getBigNumber(100)),
    ...
    describe("Short", function () {
        it("should not allow shorting if it does not return enough", async function () {
            ...
            ).to.be.revertedWith("GoldVeinPair: call failed")
        })
        ...
        it("should not allow shorting into insolvency", async function () {
            ...
            ).to.be.revertedWith("GoldVeinPair: user insolvent")
        })
        ...
        it("should limit asset availability after shorting", async function () {
            ...
            await this.pairHelper.run((cmd) => [
                ...
                cmd.depositAsset(getBigNumber(1, 8)), // Just a minimum balance for the Alpine
                cmd.as(this.bob).approveAsset(getBigNumber(1000, 8)),
                cmd.as(this.bob).depositAsset(getBigNumber(1000, 8)),
                cmd.approveCollateral(getBigNumber(100)),
                cmd.depositCollateral(getBigNumber(100)),
    ...
    describe("Cook", function () {
        it("can add 2 values to a call and receive 1 value back", async function () {
            ...
            // (25 / 2) + (37.5 / 2) = 31.25
            expect(await this.alPine.balanceOf(this.b.address, this.alice.address)).to.be.equal("3125000000")
        })

        it("reverts on a call to the Alpine", async function () {
            const ACTION_CALL = 30
            await expect(
                this.pairHelper.contract.cook(
                    [ACTION_CALL],
                    [0],
                    [defaultAbiCoder.encode(["address", "bytes", "bool", "bool", "uint8"], [this.alPine.address, "0x", false, false, 0])]
                )
            ).to.be.revertedWith("GoldVeinPair: can't call")
        })
        ...
        it("executed Alp transfer multiple", async function () {
            await this.pairHelper.run((cmd) => [
        ...
        it("allows to addAsset with approval", async function () {
            const nonce = await this.alPine.nonces(this.alice.address)
            await expect(
                await this.pairHelper.run((cmd) => [
                    cmd.approveAsset(getBigNumber(100, 8)),
                    cmd.depositAssetWithApproval(getBigNumber(100, 8), this.goldveinPair, this.alicePrivateKey, nonce),
                ])
            )
        })
    })
    ...
    describe("Liquidate", function () {
        it("should not allow open liquidate yet", async function () {
            await this.pairHelper.run((cmd) => [
                ...
                cmd.borrow(sansBorrowFee(getBigNumber(75, 8))),
                cmd.accrue(),
                cmd.do(this.alPine.connect(this.bob).deposit, this.b.address, this.bob.address, this.bob.address, getBigNumber(20, 8), 0),
            ])
            ...
            ).to.be.revertedWith("GoldVeinPair: all are solvent")
        })
        ...
        it("should allow closed liquidate", async function () {
            await this.pairHelper.run((cmd) => [
                ...
                cmd.do(this.oracle.set, "11000000000000000000000000000"),
                //cmd.do(this.a.transfer, this.luckySwapPair.address, getBigNumber(500)),
                //cmd.do(this.luckySwapPair.sync),
                cmd.updateExchangeRate(),
            ])
        ...
        it("should not allow closed liquidate with invalid swapper", async function () {
            ...
            ).to.be.revertedWith("GoldVeinPair: Invalid swapper")
        })
    })
    ...
    describe("Set Fee To", function () {
        it("Mutates fee to", async function () {
            await this.goldveinPair.setFeeTo(this.bob.address)
            expect(await this.goldveinPair.feeTo()).to.be.equal(this.bob.address)
        ...
        it("Emit LogFeeTo event if dev attempts to set fee to", async function () {
            await expect(this.goldveinPair.setFeeTo(this.bob.address)).to.emit(this.goldveinPair, "LogFeeTo").withArgs(this.bob.address)
        })

        it("Reverts if non-owner attempts to set fee to", async function () {
            await expect(this.goldveinPair.connect(this.bob).setFeeTo(this.bob.address)).to.be.revertedWith("caller is not the owner")
