...
module.exports = async function (hre) {
    ...
    let goldnuggetOwner = "0x10601b88F47e5FAfE9Da5Ac855c9E98e79903280"
    if (chainId == "1") {
        let goldnuggetOwner = "0x19B3Eb3Af5D93b77a5619b047De0EED7115A19e7"
    }
    ...
    /*
    console.log("Deploying Alpine contract")
...
*/
    const goldveinpair = (await hre.ethers.getContractFactory("GoldVeinPairMediumRiskV1")).attach(
        (await deployments.get("GoldVeinPairMediumRiskV1")).address
    )
    /*  const swapper = (await hre.ethers.getContractFactory("LuckySwapSwapperV1")).attach((await deployments.get("LuckySwapSwapperV1")).address)
     */
    const swapper = (await hre.ethers.getContractFactory("LuckySwapSwapperV1")).attach("0x1766733112408b95239aD1951925567CB1203084")
    console.log("Whitelisting Swapper")
    tx = await goldveinpair.connect(deployer).setSwapper(swapper.address, true, {
        gasLimit: 100000,
        gasPrice: finalGasPrice,
    })
    await tx.wait()

    console.log("Update GoldVeinPair Owner")
    tx = await goldveinpair.connect(deployer).transferOwnership(goldnuggetOwner, true, false, {
        gasLimit: 100000,
        gasPrice: finalGasPrice,
    })
    await tx.wait()
