...
methods {
	...
	// Alpine functions
	alPine.transfer(address token, address from, address to, uint256 share) => DISPATCHER(true)
	alPine.balanceOf(address token, address user) returns (uint256) envfree
	alPine.assumeRatio(address token, uint ratio) envfree
...
rule totalAssetElasticLeAlpineBalanceOf(method f) { // Le: less than or equal to
	...
	require alPine.balanceOf(assetInstance, currentContract) >= totalAssetElastic();
...
function validState() {
	...
	// rule totalCollateralLeAlpineBalanceOf
	require alPine.balanceOf(collateralInstance, currentContract) >= totalCollateralShare();

	// rule totalAssetElasticLeAlpineBalanceOf
	require alPine.balanceOf(assetInstance, currentContract) >= totalAssetElastic();
}
...
rule integrityOfSkimAddCollateral(address to, uint256 share, address from) {
	...
	require eAlp.msg.sender == e.msg.sender;
	require eAlp.block.number == e.block.number;
	require eAlp.block.timestamp == e.block.timestamp;
...
rule integrityOfLiquidate() {
	...
	sinvoke alPine.assumeRatio(collateralInstance, 2);

	calldataarg args;
	liquidate(e, args);

	uint256 collateralBalanceAfter = alPine.balanceOf(collateralInstance, currentContract);
	uint256 assetBalanceAfter = alPine.balanceOf(assetInstance, currentContract);
